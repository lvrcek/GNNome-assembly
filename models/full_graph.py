...
class GraphGatedGCNModel_2d(nn.Module):
    def __init__(self, node_features, edge_features, hidden_features, hidden_edge_features, num_layers, hidden_edge_scores, batch_norm, nb_pos_enc):
        super().__init__()
        #self.node_encoder = layers.NodeEncoder(node_features, hidden_features)
        self.linear_pe = nn.Linear(nb_pos_enc + 2, hidden_features) # PE + degree_in + degree_out
        ...
        self.linear1_edge = nn.Linear(edge_features, hidden_edge_features)
        self.linear2_edge = nn.Linear(hidden_edge_features, hidden_features)
        self.linear3_edge = nn.Linear(edge_features, hidden_edge_features)
        self.linear4_edge = nn.Linear(hidden_edge_features, hidden_features)
        self.gnn = layers.GraphGatedGCN_2d(num_layers, hidden_features, batch_norm)
        self.predictor = layers.ScorePredictor_2d(hidden_features, hidden_edge_scores)

    def forward(self, graph, x, e, pe):
        #x = self.node_encoder(x) # x=1
        x = self.linear_pe(pe)
        #e = self.edge_encoder(e) # e=[len,sim] default
        e_f = self.linear1_edge(e)
        e_f = torch.relu(e_f)
        e_f = self.linear2_edge(e_f)
        e_b = self.linear3_edge(e)
        e_b = torch.relu(e_b)
        e_b = self.linear4_edge(e_b)
        x, e_f, e_b = self.gnn(graph, x, e_f, e_b)
        scores = self.predictor(graph, x, e_f, e_b)
        return scores
